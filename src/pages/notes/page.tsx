...
import {
  Plus, Search, X, StickyNote, LayoutGrid, ChevronDown, Pencil, Trash2, LogOut,
  WifiOff, Moon, Sun, Download
} from "lucide-react";
...
type NoteModalState =
  | { mode: "create"; boardId?: Id<"boards"> }
  | { mode: "edit"; note: Doc<"notes"> }
  | null;

type BoardModalState =
  | { mode: "create" }
  | { mode: "edit"; board: Doc<"boards"> }
  | null;
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-8 text-center">
      <div className="fade-in-up space-y-6 max-w-md">
        ...
        <p className="text-muted-foreground text-lg">
          Твоите бележки, организирани в бордове. Бързо, сигурно, навсякъде.
        </p>
...
function NotesAppInner() {
  ...
  const notes = useQuery(
    api.notes.list,
    search ? "skip" : { boardId: activeBoardId }
  );
    ...
    async (data: { title: string; content: string; colorIndex: number }) => {
      try {
        if (noteModal?.mode === "edit") {
          ...
          toast.success("Бележката е създадена");
        }
      } catch (err) {
        const msg = err instanceof ConvexError ? (err.data as { message: string }).message : "Грешка";
        toast.error(msg);
      }
    },
    ...
    async (data: { name: string; colorIndex: number }) => {
      try {
        if (boardModal?.mode === "edit") {
          await updateBoard({ boardId: boardModal.board._id, ...data });
          toast.success("Бордът е обновен");
        } else {
          await createBoard(data);
          toast.success("Бордът е създаден");
        }
      } catch {
        toast.error("Грешка при запазване на борда");
      }
    },
  ...
  const handleDeleteBoard = useCallback(
    async (boardId: Id<"boards">) => {
      try {
        ...
        if (activeBoardId === boardId) setActiveBoardId(undefined);
        toast.success("Бордът е изтрит");
      } catch {
        toast.error("Грешка при изтриване");
      }
    },
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="sticky top-0 z-40 bg-card/80 backdrop-blur-md border-b border-border shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-sm">
            <label htmlFor={searchId} className="sr-only">Търсене</label>
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            <Input
              id={searchId}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Търси бележки..."
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {showBoardMenu && (
                <div
                  className="absolute right-0 top-full mt-1 w-64 bg-popover border border-border rounded-2xl shadow-xl z-50 py-2 fade-in-up"
                  role="menu"
                >
                  {/* All notes */}
                  <button
                    className="w-full text-left px-4 py-2.5 text-sm font-semibold hover:bg-muted transition-colors flex items-center gap-2"
                    onClick={() => { setActiveBoardId(undefined); setShowBoardMenu(false); }}
                    role="menuitem"
                  >
                  ...
                  {boards?.map((board) => {
                    ...
                    return (
                      <div key={board._id} className="flex items-center group">
                        <button
                          className="flex-1 text-left px-4 py-2.5 text-sm hover:bg-muted transition-colors flex items-center gap-2"
                          onClick={() => { setActiveBoardId(board._id); setShowBoardMenu(false); }}
                          role="menuitem"
                        >
                        ...
                        <button
                          onClick={() => { setBoardModal({ mode: "edit", board }); setShowBoardMenu(false); }}
                          className="p-2 opacity-0 group-hover:opacity-100 hover:text-primary transition-all"
                          aria-label="Редактирай борд"
                        >
                          <Pencil size={13} />
                        </button>
                        <button
                  ...
                  <div className="border-t border-border mt-1 pt-1">
                    <button
                      ...
                      onClick={() => { setBoardModal({ mode: "create" }); setShowBoardMenu(false); }}
                      role="menuitem"
                    >
                      <Plus size={14} />
                      Нов борд
            ...
            {canInstall && (
              <button
                onClick={install}
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/10 hover:bg-primary/20 text-primary text-xs font-bold transition-colors"
                aria-label="Инсталирай приложението"
              >
                <Download size={13} /> Инсталирай
              </button>
            )}
            {/* User avatar + logout */}
            {currentUser && (
        ...
        {activeBoard && (
          <div className="max-w-6xl mx-auto px-4 pb-2 flex items-center gap-2 fade-in-up">
            <span
              ...
              style={{ background: BOARD_COLORS[activeBoard.colorIndex % BOARD_COLORS.length].bg }}
            >
              <LayoutGrid size={11} />
              {activeBoard.name}
            </span>
            <button onClick={() => setActiveBoardId(undefined)} className="text-xs text-muted-foreground hover:text-foreground transition-colors">
              × Покажи всички
            </button>
          </div>
        )}
      ...
      {!isOnline && (
        <div className="slide-down sticky top-[57px] z-30 flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-950 border-b border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-200 text-sm font-semibold">
          <WifiOff size={15} />
          Офлайн режим — промените ще се синхронизират когато се свържеш
        </div>
      )}
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-6">
        ...
        {search && (
          <p className="text-sm text-muted-foreground mb-4 fade-in-up">
            {searchResults ? `${searchResults.length} резултата за „${search}"` : "Търся..."}
          </p>
        )}
        ...
        {displayNotes === undefined ? (
          <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4 space-y-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <Skeleton key={i} className="break-inside-avoid rounded-2xl" style={{ height: 120 + (i % 3) * 40 }} />
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center fade-in-up">
            ...
            <h3 className="font-bold text-lg mb-1">{search ? "Няма намерени бележки" : "Няма бележки"}</h3>
            <p className="text-muted-foreground text-sm mb-6">
              {search ? "Опитай с различни думи" : "Натисни + за да добавиш бележка"}
            </p>
            {!search && (
              <Button onClick={() => setNoteModal({ mode: "create", boardId: activeBoardId })} className="rounded-xl">
                <Plus size={16} className="mr-1" /> Нова бележка
              </Button>
            )}
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {pinnedNotes.length > 0 && (
              <section aria-label="Закачени бележки">
                ...
                <SortableNoteGrid
                  notes={pinnedNotes}
                  onEdit={(n) => setNoteModal({ mode: "edit", note: n })}
      ...
      <button
        ...
        aria-label="Нова бележка"
        title="Нова бележка (Ctrl+Enter за запазване)"
      ...
      {showBoardMenu && (
        <div className="fixed inset-0 z-40" onClick={() => setShowBoardMenu(false)} aria-hidden="true" />
      )}

      {/* Modals */}
      {noteModal && (
        <NoteModal
          note={noteModal.mode === "edit" ? noteModal.note : null}
